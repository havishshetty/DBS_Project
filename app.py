...
SELECT_COMMENT_QUERY = "SELECT * FROM Comments WHERE ProjectID = %s;"
...
@app.route("/api/task_status/<int:project_id>", methods=["GET"])
def get_task(project_id):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(SELECT_TASK_QUERY, (project_id,))
            ...
            if task:
                ...
                return jsonify({"Status": "Not Yet Started"}), 404
...
@app.route("/api/comment_project/<int:project_id>", methods=["GET"])
def get_comment(project_id):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute("SELECT * FROM comments WHERE projectId = %s", (project_id,))
            ...
            if comment:
                ...
                return jsonify({"CommentText": "No Comment Yet"}), 404
